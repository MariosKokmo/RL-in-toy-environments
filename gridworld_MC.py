...
class world():
    ...
    def transition(self, state, action):
        ...
        height_index = len(self.grid)-1
        width_index = len(self.grid[0])-1
        done = False
        ...
        if action == 0:
            new_state = self.grid[max(0,state[0]-1)][state[1]]
            reward = self.rewards[max(0,state[0]-1)][state[1]]
        elif action == 1: # if down
            new_state = self.grid[min(height_index,state[0]+1)][state[1]]
            reward = self.rewards[min(height_index,state[0]+1)][state[1]]
        elif action == 2: # if right
            new_state = self.grid[state[0]][min(state[1]+1,width_index)]
            reward = self.rewards[state[0]][min(state[1]+1,width_index)]
        else: # left
            new_state = self.grid[state[0]][max(state[1]-1,0)]
            reward = self.rewards[state[0]][max(state[1]-1,0)]
        ...
        if (state == self.final_state) or (self.rewards[state[0],state[1]] == self.reward[1]):
            done = True
